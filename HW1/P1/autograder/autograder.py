...
import json
from HW1.P1.mytorch import Identity, Sigmoid, Tanh, ReLU, Softmax
from HW1.P1.mytorch import MSELoss, CrossEntropyLoss
from HW1.P1.mytorch import Linear
from HW1.P1.mytorch import BatchNorm1d
from HW1.P1.mytorch import SGD
from HW1.P1.models.model import MLP0, MLP1
from .autograder_flags import *
...
if DEBUG_AND_GRADE_MLP0:
    ...
    for i in range(n_tests):
        ...
        Z1_tensor = torch_linear(A0_tensor)
        Z1 = Z1_tensor.detach().numpy()
        A1_tensor = torch.relu(Z1_tensor)
...
if DEBUG_AND_GRADE_MLP1:
    ...
    for i in range(n_tests):
        ...
        Z1_tensor = torch_linear0(A0_tensor)
        Z1 = Z1_tensor.detach().numpy()
        A1_tensor = torch.relu(Z1_tensor)
        A1 = A1_tensor.detach().numpy()
        A1_tensor_copy = torch.tensor(A1, requires_grad=True)
        Z2_tensor = torch_linear1(A1_tensor_copy)
        Z2 = Z2_tensor.detach().numpy()
        A2_tensor = torch.relu(Z2_tensor)
